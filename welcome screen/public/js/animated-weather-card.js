...
function changeWeather(weather)
{
	...
	switch(weather.type)
	{
		case 'sun':
			...
			TweenMax.to(sunburst.node, 4, {scale: 1, opacity: 0.8, x: sizes.card.width / 2, y: (sizes.card.height/2) + (sizes.card.offset.top), ease: Power2.easeInOut});
			break;
		default:
			TweenMax.to(sun.node, 2, {x: sizes.card.width / 2, y: -100, leafCount: 0, ease: Power2.easeInOut});
			TweenMax.to(sunburst.node, 2, {scale: 0.4, opacity: 0, x: sizes.card.width / 2, y: (sizes.card.height/2) + (sizes.card.offset.top), ease: Power2.easeInOut});
			break;
	}	
