...
<body id="page-top">
    ...
    <div class="py-4">
            <div class="container">
                ...
                <p>การดำเนินการสิ้นเสร็จแล้ว คุณสามารถแก้ไข / เข้าสู่หน้า View ของแต่ละ Event ได้จากตาราง Event / QR code ของแต่ละ Event </p>
                <hr>
        ...
        <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="https://dev.invp.co.th/intern/event/">Main</a></li>
        <li class="breadcrumb-item active" aria-current="page">Tag Wait</li>
        </ol>
        </nav>

                <span class="badge rounded-pill bg-dark text-white my-2" style="font-size:20px;">Event Info</span>
                        ...
                        <td><a href="{{url('/event/create')}}" class="btn btn-success border border-dark md-3" style="width:148px;height:35px"><b><i class="fa fa-plus-circle" aria-hidden="true"></i> Create Event</b></a></td>
                        <td><a href="{{url('/event/trash')}}" class="btn btn-warning border border-dark md-3" style="width:100px;height:35px"><b><i class="fas fa-archive"></i> Trash</b></a></td>
                        
                        <!-- Sort by section  -->
                        <td>
                        <button class="btn btn-secondary dropdown-toggle" type="button" style="width:110px;height:35px" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <b><i class="fas fa-align-center"></i> Sort by</b>
                        </button>
                        ...
                        <li><a class="dropdown-item"  href="{{url('/event/tagwaitsort')}}">Wait Sort</a></li>
                        </div>
            ...
            {{$tagwait_sort ->links("pagination::bootstrap-4")}}
                ...
                <tbody>
                    <!-- แปลง event เป็น row -->
                    @foreach($tagwait_sort as $row)
                <tr align="center">

                    <td style="display:none" >{{$row->uuid}}</td>
                    @if($row->status=="END")
                    <td><button class="badge badge-dark" style="font-size:10px"><b>{{$row->status}}</b></button></td>
                    @else
                    <td><button class="badge badge-success" style="font-size:10px"><b>{{$row->status}}</b></button></td>
                    @endif
                    <td><a href="{{url('/event/view/'.$row->NameEvent )}}"><h6><b>{{$row->NameEvent}}</b></h6></td></a>
                    <td><b>{{$row->datestart}}</b></td>
                    <td><b>{{$row->dateend}}</b></td>
                    <td><b>{{$row->timestart}}</b></td>
                    <td><b>{{$row->timeend}}</b></td>
                    <td><b>{{$row->location_event}}</b></td>
                    
                    <!-- เลข id จะตาม id แถวที่กด -->
                
                    <td><a href="{{url('/event/edit/'.$row->id)}}"  class="btn btn-secondary btn-circle"><i class="fas fa-edit"></i></a></td>
                    <td><a href="{{url('/event/softdelete/'.$row->id )}}" onclick="return confirm('คุณต้องการย้ายข้อมูล Event นี้ไปถังขยะหรือไม่ ?')" class="btn btn-danger btn-circle"><i class="fas fa-trash"></i></a></td>
                    <td><a href="{{url('/event/view/'.$row->NameEvent )}}"  class="btn btn-info btn-circle"><i class="fas fa-info-circle"></i></a></td>
                    <td><a href="{{url('/event/qrcode/'.$row->uuid )}}"  class="btn btn-primary btn-circle"><i class="fa fa-qrcode"></i></a></td>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>ยืนยันการลบข้อมูลไปยังถังขยะหรือไม่?</p>
      </div>
      
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>
        <a href="{{url('/event/softdelete/'.$row->id)}}" class="btn btn-primary">ยืนยัน</a>
      </div>
    </div>
  </div>
</div>
