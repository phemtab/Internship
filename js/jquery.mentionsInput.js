        ...
        function populateDropdown(query, results) {
            ...
            if(!settings.allowRepeat) {
                ...
                results = _.reject(results, function (item) {
                    return _.include(mentionValues, item.firstname);
                });
            }
            ...
            _.each(results, function (item, index) {
                ...
                autocompleteItemCollection[itemUid] = _.extend({}, item, {value: item.firstname}); //Inserts the new item to autocompleteItemCollection

                var elmListItem = $(settings.templates.autocompleteListItem({
                    'id'      : utils.htmlEncode(item.id),
                    'display' : utils.htmlEncode(item.name),
                    'type'    : utils.htmlEncode(item.type),
                    'content' : utils.highlightTerm(utils.htmlEncode((item.display ? item.display : item.firstname)), query)
